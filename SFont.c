...
#ifdef ZIPIT_Z2
    // Support fixed width GMU 9 fonts (or pass in char width.  eg.  -W8)
    // Maybe think about supporting 16x16 "rectangle of chars" style png font.
    // Also, rockbox formatted raster fonts may be available.
    if (i == 0)
    {
      int w = Surface->w / 223;
      int r = Surface->w / Surface->h;
      if ((r < 130) || (r > 140) || (w < 5))
	w = SFont_DefaultCharWidth;
      if (w < 5)
	w = 8;
      x = 1;
      while (x < Surface->w) {
	Font->CharPos[i++]=x;
	Font->CharPos[i++]=x; //Font->CharPos[i++]=x+1;
	x += w;
      }
    }

    //    printf("(%dx%d) i=%d, %d,%d ... %d,%d\n", Surface->w, Surface->h,
    //	   i, Font->CharPos[0],Font->CharPos[1],Font->CharPos[2],Font->CharPos[3]);
#endif
    Font->MaxPos = x-1;
...
{
#ifdef ZIPIT_Z2
    // Allow full 8bit latin1 charset.
